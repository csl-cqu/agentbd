...
def main(test_data_path, output_csv_path, llm, num):
    ...
    if llm == "qwen":
        local_model_path = "xxx/NLPLab/AgentsBD/LLM/Qwen/Qwen2.5-7B-Instruct"
        lora_adapter_path = "./qwen_7b_"+ str(num) + "_de/final_lora_adapters" #  the adapters of qwen2.5-7b-instruct
    elif llm == "llama":
        local_model_path = "xxx/NLPLab/AgentsBD/LLM/LLM-Research/Meta-Llama-3-8B-Instruct"
        lora_adapter_path = "./llama3_8b_"+ str(num) + "_de/final_lora_adapters" #  the adapters of llama3-8b-instruct
    else:
    ...
    for example in tqdm(test_dataset, desc="Inferencing texts"):
        ...
        prompt = f"### Instruction: Determine whether the following text contains a backdoor. If so, response the type of the backdoor. If not, response the 'clean'. \n ### Input: {text}  \n ### Response:"

        """Performs inference on the given text."""
        inputs = tokenizer(prompt, return_tensors="pt", padding=True, truncation=True).to(model.device)
        with torch.no_grad():
            outputs = model.generate(**inputs, max_new_tokens=200, num_beams=5, early_stopping=True) # 
        ...
        results.append({'': id, '0': text, '1': label, '2': target_label, '3':attack_type, '4':extracted_text})
